...
# --- LOGIN SYSTEM START ---
# Hardcoded credentials (Aap yahan apna username/password set kar sakte hain)
USER_CREDENTIALS = {
    "admin": "12345",
    "ASHWAJIT": "pass123",
    "HARSHAL": "Shalvi@3009"
...
if not st.session_state["authenticated"]:
    login()
    st.stop()  # Iske aage ka code nahi chalega jab tak login na ho!
...
if not df_data.empty:
    ...
    if not top_movers.empty:
        ...
        st.dataframe(
            top_movers[['Sector', 'Symbol', 'Price', 'Change %', 'RSI (14)', '20-EMA', 'EMA Trend', 'R Fact', 'Signal']],
    ...
    # Sector Drill-down Table with Filter Option
    ...
    st.dataframe(
        sector_stocks[['Symbol', 'Price', 'Change %', 'RSI (14)', '20-EMA', 'EMA Trend', 'R Fact', 'Signal']].sort_values(by='Change %', ascending=False),
        use_container_width=True,
        hide_index=True
    )
